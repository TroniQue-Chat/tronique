...
import DexTradeHomescreen from "./DexTradeHomescreen";
import DexTradeMessageHistory from "./DexTradeMessageHistory";
import {
...
const DexChatscreen: React.FC = () => {
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      const aiRes = await generateAndRunSQLTrades(msg);
      const { df } = aiRes;
      ...
      const plotlyRes = await generatePlotlyFigureTrades(msg);
    ...
    <div className={`chatscreen z-10 ${showSideBar ? "w-[80%]" : "w-[100%]"}`}>
      {messageHistory?.length === 1 ? (
        <DexTradeHomescreen
          ...
          generateAndRunSQLTrades={generateAndRunSQLTrades}
          loading={loading}
        />
      ) : (
        <DexTradeMessageHistory
          runSQL={generateAndRunSQLTrades}
